...
> 如果 $f$ 在 $x^{*}$ 附近满足 $|f'(x)| \le L < 1$,那么每一步误差都**至少乘以 $L$**,轨道指数级地收缩进不动点。
...
认出这些分数了吗?全是**斐波那契相邻两项之比** $F_n / F_{n+1}$——上一篇用比奈公式证明的事,现在它自己走出来了。

为什么?归纳即可。设 $x_n = F_n/F_{n+1}$(初值 $x_0 = 1 = F_1/F_2$ 符合),则

$$x_{n+1} = \frac{1}{1 + x_n} = \frac{1}{1 + F_n/F_{n+1}} = \frac{F_{n+1}}{F_n + F_{n+1}} = \frac{F_{n+1}}{F_{n+2}}$$
...
斐波那契平方项与相邻项乘积之间永远差 $\pm 1$——这甚至给"斐波那契数没有平方数(除 1 外)"之类的数论结论铺了路。
...
1. **函数迭代**(本文前半):$F_n/F_{n+1}$ 的轨道,奔不动点 $\varphi - 1$;
