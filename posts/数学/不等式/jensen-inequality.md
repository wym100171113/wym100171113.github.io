...
> $f$ 二次可导，则 $f$ 凸 ⟺ $f''(x) \ge 0$ 处处成立；若 $f'' > 0$ 处处成立，则 $f$ 严格凸（反向不成立：$f(x) = x^4$ 严格凸，但 $f''(0) = 0$）。
...
于是判据落成：**$f'' \ge 0$ 凸，$f'' \le 0$ 凹**。第三节的例题全靠它：$x\ln x$ 的 $f'' = 1/x > 0$，$-\ln x$ 的 $f'' = 1/x^2 > 0$，$e^x$ 的 $f'' = e^x > 0$，幂平均链里的 $x^2$、$\frac{1}{x}$ 也不例外。
...
>
> ```xml
> <!-- 抛物线: 采样 ~480 点折线, 视觉上与自适应采样无差别 -->
> <polyline points="x=(-2.4,2.4,0.008) f=y*y" fill="none" stroke="#3c6eb9" stroke-width="3"/>
> <!-- 弦/切线: 直线或方程直接画; 中点垂线标出 f(m) 与 (f(x)+f(y))/2 的差距 -->
> <!-- 文字: 浏览器字体, 无需嵌字形 -->
> <text font-family="PingFang SC, STHeiti, sans-serif" font-size="24">(m, f(m))</text>
> ```
>
> - 图 2 切线 $y = 2x_0 x - x_0^2$（$x_0 = 0.8$）；图 3 弦斜率 $k=\dfrac{f(x_2)-f(x_1)}{x_2-x_1}$；
> - 文字统一用 `text-anchor` 对齐到点（`middle`/`start`），同列标签错开行距避免粘连，如端点 `x`/`y` 与中点 `x_1,x_2,x_3` 都放在刻度数字下一行；
> - 坐标轴刻度、网格用细线，曲线、弦、点、标注分层绘制，配色与站内其他示意图一致（白底、墨线、一抹蓝）。
