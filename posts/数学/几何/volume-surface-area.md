...
![柱体与锥体的侧面展开：柱体侧面展开成矩形（底面周长×高），锥体侧面展开成扇形（半径是斜高）|600](../../assets/img/lateral-unfold.svg)
...
- **转轴必须在图形所在的平面内（共面），且不能穿过图形。** 轴若不在图形平面内，旋转时图形会斜着"拧"出去，重心到轴的距离不断变化、轨迹不再是圆，"走过的路程 $= 2\pi\bar R$"这一步直接失效；轴若穿过图形，旋转自交、体积被重复计算。圆环体的小圆与轴共面、相距 $R > r$，两条都满足；
- **默认转满一整圈。** 若只旋转 $\theta$（弧度），体积 = 面积 × 重心走过的弧长 $= A \cdot \theta \bar R$；
- **表面积版本里的"重心"是生成曲线的重心，不是区域的重心。** 曲线重心与区域重心一般不在同一点，拿错就错。圆环体里两者恰好重合（都在小圆圆心），所以下面的体积与表面积算起来都干净利落——但这纯属圆环体的好运，不是定理的常态。
