...
第六节的平面方程 $\vec n\cdot(\vec X-\vec X_0)=0$ 展开、合并常数，就是一般式 $Ax+By+Cz+D=0$。它的读法只有一句：**系数 $(A,B,C)$ 就是法向量**——读方程等于直接读出"哪个方向垂直于这个面"；剩下的 $D$ 管偏移（原点到平面的距离是 $|D|/\sqrt{A^2+B^2+C^2}$）。

### 距离、投影点、对称点：一次推导，三个公式

设 $F(x,y,z)=Ax+By+Cz+D$，平面上任一点满足 $F=0$。给点 $P_0(x_0,y_0,z_0)$，从它出发**沿法线方向走**：$P(t)=P_0+t\,(A,B,C)$。要求 $P(t)$ 落在平面上：

$$F(P_0)+t\,(A^2+B^2+C^2)=0\ \Longrightarrow\ t=-\frac{F(P_0)}{A^2+B^2+C^2}$$

这一段走了多远？$|t|\cdot|(A,B,C)|$，于是

$$\boxed{\ d=\frac{|\,Ax_0+By_0+Cz_0+D\,|}{\sqrt{A^2+B^2+C^2}}\ }$$

注意分子：**就是把这个点代进方程左端**。所以 $F$ 的函数值不是算错才冒出来的东西——它是带符号的、没单位化的"离面程度"。（更深一层的读法：$\nabla F=(A,B,C)$ 处处垂直于等值面 $F=0$，$|\nabla F|$ 是 $F$ 沿法向的变化率，所以垂直距离 = 函数值差 ÷ 变化率。这个"梯度垂直于等值面"的模式，和物理里 $\vec E=-\nabla\varphi$ 是同一个。）

而刚才那个 $t$ 顺便把另外两件事也办了：

- **投影点（垂足）**：$P'=P_0+t\,(A,B,C)=P_0-\dfrac{F(P_0)}{A^2+B^2+C^2}\,(A,B,C)$；
- **对称点（关于平面的镜像）**：垂足是 $P_0$ 与镜像的中点，所以 $P''=P_0+2t\,(A,B,C)$。

一句话：**投影走一半，镜像走全程。**

算一个：平面 $x-2y+2z-5=0$，点 $P(2,2,2)$。$F(P)=2-4+4-5=-3$，$A^2+B^2+C^2=9$，$t=\dfrac13$：

$$d=\frac{|-3|}{3}=1,\qquad P'=\left(\tfrac73,\tfrac43,\tfrac83\right),\qquad P''=\left(\tfrac83,\tfrac23,\tfrac{10}3\right)$$

代回验证：$\tfrac73-2\cdot\tfrac43+2\cdot\tfrac83-5=\tfrac{15}{3}-5=0$ ✓（更多复核见文末自检第 7 条）。

### 顺手收下的几个推论

- **两平行平面的距离**：$Ax+By+Cz+D_1=0$ 与 $Ax+By+Cz+D_2=0$ 相距 $\dfrac{|D_1-D_2|}{\sqrt{A^2+B^2+C^2}}$（在其中一个面上任取一点，代入距离公式即得）；
- **两平面交线的方向**：$\vec v=\vec n_1\times\vec n_2$——交线既在面 $\pi_1$ 里又在面 $\pi_2$ 里，所以同时垂直于两个法向量；
- **平面束**：过交线的所有平面是 $\lambda F_1+\mu F_2=0$——第一节那个"过一条直线的无数个平面"，现在有了显式写法；
- **点到直线的距离**：$d=\dfrac{|\vec v\times\vec{P_1P_0}|}{|\vec v|}$（$\vec v$ 是方向向量，$P_1$ 在线上）——叉积的模是平行四边形面积，除以底就是高。
...
| 单叶双曲面 | $x^2+y^2-z^2=1$ | 椭圆（$c=0$ 时最细，是"腰"） |
| 双叶双曲面 | $x^2+y^2-z^2=-1$ | $\lvert c\rvert<1$ 时什么都切不到——曲面断成两叶 |
| 双曲抛物面（马鞍面） | $z=\dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}$ | 双曲线；$c=0$ 时是两条交叉直线 |

截痕法其实就是这一篇反复用的降维：**研究空间的曲面，先看一族平面切出来的平面曲线**——和当年用投影把空间压回平面是同一个手势。

二次曲面里最反直觉的事实：单叶双曲面和马鞍面虽然浑身是曲线，却都是**直纹面**——整个曲面可以由直线织成。看单叶双曲面 $x^2+y^2-z^2=1$：过腰圆上任一点 $(\cos\theta,\sin\theta,0)$，直线

$$(x,y,z)=(\cos\theta-t\sin\theta,\ \sin\theta+t\cos\theta,\ t)$$

整条躺在曲面上（代入验证：$x^2+y^2=(1+t^2)=1+z^2$ ✓）。每个 $\theta$ 给一条，就织出整张曲面。

![单叶双曲面是直纹面：两族方向相反的直线，织出一张弯曲的曲面——冷却塔的形状（Mathematica 3D 绘制）](../../assets/img/quadric-ruled.png)

这不是数学游戏：发电厂的双曲面冷却塔，就是用**直的**钢筋织出曲的塔身——省料、好施工，还自带结构强度。

最后往 $n$ 维望一眼。$n$ 维空间里的超平面是 $a_1x_1+\cdots+a_nx_n=b$：第一节的自由度账原样成立（$n$ 个独立点确定一个超平面），距离公式也原样成立：

$$d=\frac{|\,a_1x_1^{(0)}+\cdots+a_nx_n^{(0)}-b\,|}{\sqrt{a_1^2+\cdots+a_n^2}}$$

**高维解析几何没有新公式，只有更大的 $n$。** 当然直觉会失效——四维空间里两个"二维平面"可以只交于一个点，这是三维眼睛看不见的第一站——但公式照样工作。这就是坐标化的终极回报：维数变了，程序不用变。
