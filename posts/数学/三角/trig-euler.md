...
- 下一篇：[《单位根与正十七边形：高斯的那一天》](post.html?slug=roots-of-unity)——单位根的两种身份、分圆多项式，以及正十七边形与高斯的日记。
...
>
> **4. 一个独立的验证：$\cos 5\theta$。** 由递推算出 $T_5=16x^5-20x^3+5x$。令 $x=\cos 36^\circ$，则 $\cos 180^\circ=-1$，于是 $16x^5-20x^3+5x+1=0$。分解得 $(x+1)(4x^2-2x-1)^2=0$，其中 $4x^2-2x-1=0$ 的正根为 $\frac{1+\sqrt5}4=\cos 36^\circ=\frac{\varphi}2$，而 $\varphi$ 正是黄金分割。一条从多倍角出发的路线，绕回了[《斐波那契数列与黄金分割》](post.html?slug=fibonacci)里的那个常数。
>
