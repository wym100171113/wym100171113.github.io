...
## 六、几何染色:相似比为 1995 的同色三角形(1995 高联二试第 4 题)
...
(*另一个网格证法:先证明平面上必有同色直角三角形,补成矩形,把每条边 1995 等分,再用"必有相邻分点同色"逐行下探,找到一个 $1/1995$ 缩小的同色直角三角形。细节见附录。*)
...
(配图见下;这里先看清结构。)抽屉在这一节干两件事:先在每层内部开一刀——5 个点染两色,必有一色至少 3 个;再在同向的几层之间开一刀——几个"多数色"必有两层撞在一起(见事实 2 之后)。

**事实 1**:正五边形的任意 4 个顶点都在它的外接圆上。所以若某个五边形里有 $\ge 4$ 个同色顶点,立即得证;剩下只需对付"每层都是 3 红 2 蓝或 2 红 3 蓝"的局面——**注意各层的多数色未必相同**,这正是第二把抽屉要处理的(见下)。届时全局换一下颜色名,锚点层的三个红点只可能是 **(i) 三个连续**(如 $A_1B_1C_1$),或 **(ii) 三个非连续**(如 $A_1C_1D_1$)——两类都要打发。
...
有了两件事实,还差一步:**给 $P_1$ 配一个同为 3 红 2 蓝的同向层**。各层多数色未必一致,若 $P_6$ 偏偏是 2 红 3 蓝,下面的情形表就接不上了。好在 $P_1,P_6,P_{11}$ 三层两两同向($P_{11}$ 相对 $P_1$ 转了整一圈 $360°$),三个多数色落进两种颜色,必有两层撞色——这是本节的第二把抽屉。把撞色的那一对全局换名使共同多数叫红,再重新编号记作 $P_1$ 与 $P_6$(论证只用"同向孪生"的结构,三层里任取一对都一样)。剩下的是一步逼供:颜色只要在第 6 层的 $D_6,E_6$ 两个位置对上号,等腰梯形立刻交出同色四点。
...
每个子情形都当场冒出一个同色等腰梯形——**三个连续红,情形表本身只动用 $P_1$ 与 $P_6$ 两层**(选这对时才问过 $P_{11}$ 的多数色)。三个非连续红的情形几乎一样($A_1D_1B_6C_6$、$A_1D_1A_6D_6$ 这类全红,或 $B_1E_1C_6D_6$ 全蓝),完整分支放附录 B。

这一节的抽屉开了两刀:第一刀,每层 5 点染两色必有 $\ge 3$ 同色;第二刀,$P_1,P_6,P_{11}$ 的三个多数色必有两层相撞——没有它,"$P_6$ 也是 3 红 2 蓝"就只是一厢情愿(这不是多虑:$P_1$ 三红二蓝、$P_6$ 二红三蓝时,光靠这对层确实可能凑不出同色四点)。多的第四点是**事实 1** 白送的;三红是连续还是非连续,也只有两类。剩下的全交给几何——**隔五层的孪生五边形**把"平行"变成"共圆",颜色连推都不用推,当场交代。

**附:官方解答更快,三刀抽屉收官**。第一刀:五边形是**奇圈**,两色染色必有相邻两点同色(否则颜色沿圈交替,绕一圈回到起点就自相矛盾),于是每层选出一条单色边,共得 11 条。第二刀:这 11 条边只落在 5 个方向里——相邻层相对旋转 $36°$,而每层各边的方向集合在 $36°$ 旋转下不动——故必有 3 条互相平行;而任何两条平行边,关于公共弧中点所在半径反射两腰互换,张成等腰梯形,必共圆。第三刀:3 条单色边只有 2 种颜色,必有 2 条同色——这两条平行同色的边当场拼出四顶点同色的圆内接四边形。证毕,连哪层 3 红 2 蓝都不必过问。
...
**$n = 910$ 不够**:把 15 道题的 455 个三元组,每个三元组安排 2 名学生专答(共 $455 \times 2 = 910$ 人,第 $i$ 人恰好答对其负责的三道题)。于是任何 12 人得分和 $= 36$ ✓ 条件满足,但没有任何三道题有 3 人同对 ✗。再从这 910 人中任取 $n$($13 \le n \le 910$)人,反例原样保留——每人仍恰得 3 分、每三元组仍至多 2 人——所以 911 以下一律不行,$n$ 必须 $\ge 911$。
...
- **和边** $\{a, n-a\}$:对 $33 \le n \le 69$,$a$ 从 $\max(1,n-50)$ 取到 $\lfloor (n-1)/2 \rfloor$,这些边**两两不共顶点**——否则某顶点 $x=a_1=n-a_2$ 同时属于两条边,推出 $n=a_1+a_2 \le 2\lfloor (n-1)/2 \rfloor \le n-1$,矛盾。和边是真正的匹配,至少 $\lfloor (33-1)/2 \rfloor = 16$ 条(最少值 16 在两端附近取到:$n=33,34$ 与 $n=68,69$ 各恰 16 条,中段更多),故 $\alpha \le 50 - 16 = 34$ ✓;
...
> ### A. 1995 高联的网格证法(法一)
> 
> 先证平面上必有同色直角三角形。取一条直线 $l$,其上必有两点 $P,Q$ 同色(设红色)。过 $P,Q$ 作 $l$ 的垂线 $l_1,l_2$。若 $l_1$ 上还有红点 $R$,则 $\triangle PQR$;若 $l_2$ 上还有红点 $S$,则 $\triangle PQS$——两者都是红色直角三角形;否则,$l_1,l_2$ 上除 $P,Q$ 外全是蓝点,取 $l_1$ 上蓝点 $R,S$,过 $R$ 作 $l_1$ 的垂线交 $l_2$ 于蓝点 $T$,则 $\triangle RST$ 蓝色且直角 ✓。
> 
> 设 $\triangle ABC$ 是同色直角三角形,$\angle B = 90°$,补成矩形 $ABCD$(记 $|AB| = a$、$|BC| = b$)。把每条边 $n = 1995$ 等分,得到 $n^2$ 个小矩形。边 $AB$ 上共 $n+1 = 1996$ 个分点:$n$ 是奇数,若相邻分点全部异色,则颜色沿边交替,$A$ 与 $B$ 异色,与 $\triangle ABC$ 同色矛盾——所以必有相邻同色分点 $E,F$。
> 
> 沿 $EF$ 所在竖条逐行往下看:每行看"上边两点颜色"。若某行下边两点中有与上边同色的,立刻得到同色小直角三角形(直角在格点上,两直角边分别是 $a/n$ 与 $b/n$);若下边两点都换成了另一色,则它们彼此同色,继续下一行。走到底仍没找到,则这竖条在每一层网格线上的左右两点都同色——即每行的上下两条水平边各自单色。对 $BC$ 边同样处理(横条):同理,若一路扫完仍未找到,则横条在每一列网格线上的上下两点都同色。于是竖条与横条交界处的 $a/n \times b/n$ 小矩形 $GHNM$:上、下两条边由竖条性质各自单色,左、右两条边由横条性质各自单色,故四角全部同色——取其中半个小矩形 $\triangle MNH$,即得同色直角三角形(两直角边分别是 $a/n$ 与 $b/n$),与 $\triangle ABC$ 相似、比 $1/1995$ ✓。
> 
...
> 
> 表中每个四边形都以一对平行弦为底(如 $B_1E_1 \parallel C_6D_6$:两弦弧中点同在 $180°$ 方向),关于对应半径反射两腰互换,是等腰梯形,故共圆。注意演示只用到 $P_1$ 与 $P_6$;其余九个五边形并未上场——$P_{11}$ 唯一的职责是在多数色配对的第二次抽屉里当第三人:$P_1$ 与 $P_6$ 多数色相反时,与 $P_{11}$ 同多数的那一对才是可用搭档。真正的难点不在抽屉,而在看出 $P_6$ 是 $P_1$ 的"同向孪生"。
> 
...
> - 2011 东南:$n \le 32$ 用差边图的最大匹配(链的并,逐条数链长,最小 17、在 $n=17$ 取到),$33 \le n \le 69$ 用和匹配($\ge 16$ 条,最少值 16 在 $n=33,34,68,69$ 取到),$n \ge 70$ 反例 $\{1,\dots,35\}$。