...
一样东西只有 $n$ 个抽屉,却要放 $n+1$ 个物体——总有一个抽屉至少装两个。这个事实平凡到什么程度呢?一年最多 366 天(闰年),全校 367 名学生,必有两人同一天生日,不需要任何别的知识。但它有个吓人的名字清单:**鸽巢原理**、**抽屉原理**、**狄利克雷原理**(1834 年,Dirichlet 用"抽屉"做过表述,英语世界则叫 pigeonhole)。名字越吓人,越说明它好用:当你把一个问题翻译成"许多物体放进少数抽屉",答案往往就自己跳出来了。
...
**关键观察**:必有 $a_{n+2} \ge 2a_n$。否则 $a_n < a_{n+1} < a_{n+2} < 2a_n$,这中间三个数两两不可能有整除关系:若其中 $a_i \mid a_j$($i<j$),则 $a_j \ge 2a_i > 2a_n$,但 $a_j < 2a_n$,矛盾。三个两两不整除的元素违反了条件 ✗。所以必然 $a_{n+2} \ge 2a_n$。
...
> 从 1, 2, 3, \dots, 2026 中删去一些数,使得剩下的数中任何一个数都不等于其余任意两个不同的数的积。问最少要删去多少个数。
...
- 若 $s = 1$:则 $K$ 里任何别的数 $x$ 都满足 $x = 1 \cdot x$,违反条件——除非 $K = \{1\}$,那是删了 2025 个,更差;
...
同向层之间,两条弦只要"弧中点落在同一半径上"就互相平行:例如 $A_1C_1$ 的弧中点在半径 $OB_1$ 上,$D_6E_6$ 的弧中点也在 $OB_1$ 上,故 $A_1C_1 \parallel D_6E_6$;同理 $A_1B_1 \parallel C_6E_6$。而关于直线 $OB_1$ 的反射把 $A_1$ 映到 $C_1$、把 $D_6$ 映到 $E_6$,所以四边形 $A_1C_1E_6D_6$ 的两腰相等——它是**等腰梯形**,必有外接圆。下图的红色四边形 $A_1C_1D_6E_6$ 就是这么一对平行弦。

![嵌套正五边形:隔 5 层同向(P₁ 与 P₆,转了 180°),对应弦互相平行;红线四边形 A₁C₁D₆E₆ 中 A₁C₁ ∥ D₆E₆,等腰梯形 → 四点共圆](../../assets/img/pigeonhole-pentagons.svg)
...
> - 2026 删数:阈值 45 来自 $45 \times 46 = 2070 > 2026$;一般地,对 $\{1,\dots,N\}$,$m = \lceil (\sqrt{4N+1}+1)/2 \rceil$(即最小 $m$ 使 $m(m+1) > N$)时保留 $\{m,\dots,N\}$ 通常是最优构造,下界用"最小保留数 $s$ 的倍数区间计数"。
